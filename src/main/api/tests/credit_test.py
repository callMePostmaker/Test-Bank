...
class TestCredit:
    ...
    def test_invalid_credit(self, api_manager, create_creditor_request: CreateCreditorRequest, create_creditor_account_user_id):
        credit_account_request = UserCreditRequest(
            accountId=create_creditor_account_user_id + 1,
            amount=randint(500000, 1500000) / 100,
            termMonths=randint(6, 24)
        )
        
        response = api_manager.user_steps.invalid_credit_request(create_creditor_request, credit_account_request)
